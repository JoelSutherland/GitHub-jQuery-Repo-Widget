;jQuery(document).ready(function($){
	...
	$('.github-widget').each(function(){
        ...
        var vendorName = repo.split('/')[0],  repoUrl = "http://github.com/" + repo;
		...
		var $widget = $(
                ...
                +'/'
                +'<a class="repo" href="' + repoUrl + '" title="' + repoUrl + '">' + repo.split('/')[1] + '</a>'
                +'</h3>'
		...
		$.getJSON(
			...
			function(results) {
				...
				$widget.find('.updated').html(
                    'Latest commit to the <strong>' + repo.default_branch + '</strong> branch on ' + pushed_at
                );
